...
/* USER CODE BEGIN PD */
#define CONFIGS 15
#define MEM1_ADDRESS 0b10100000
#define MEM2_ADDRESS 0b10100010
...
osThreadId_t dmxTaskHandle;
const osThreadAttr_t dmxTask_attributes = {
		.name = "dmxTask",
		.stack_size = 256 * 4,
		.priority = (osPriority_t) osPriorityHigh,
};
...
void noteOn(uint8_t channel, uint8_t pitch, uint8_t velocity) {
	...
	uint8_t dmxData[2]; // 1st value = dmx channel, 2nd value = dmx value
	...
	for (uint16_t i = entries[0]; i < entries[1]; i = i + 2) {
		HAL_I2C_Mem_Read(&hi2c1, MEM2_ADDRESS, i, I2C_MEMADD_SIZE_16BIT, dmxData, 2, 100);
		dmxWrite(dmxData[0], dmxData[1]);
	}
}
...
void controlChange(uint8_t channel, uint8_t controller, uint8_t value) {
	uint8_t addresses[4]; // buffer to get the 16bit addresses of the entries:
	uint16_t entries[2]; // 1st value = address of 1st entry, 2nd value = address of 1st entry of next pitch
	uint8_t dmxChannel; // dmx channel
	HAL_I2C_Mem_Read(hi2c, MEM1_ADDRESS, CONFIGS + channel * N_TYPES * N_PITCHES * 2 + pitch * 2 + 2 * N_PITCHES * 2, I2C_MEMADD_SIZE_16BIT, addresses, 4, 100);
	entries[0] = (uint16_t)(addresses[0] << 8 + addresses[1]);
	entries[1] = (uint16_t)(addresses[2] << 8 + addresses[3]);
	for (uint16_t i = entries[0]; i < entries[1]; i = i + 2) {
		HAL_I2C_Mem_Read(&hi2c1, MEM2_ADDRESS, i, I2C_MEMADD_SIZE_16BIT, dmxData, 2, 100);
...
int main(void)
{
  ...
  /* USER CODE BEGIN RTOS_THREADS */
	...
	dmxTaskHandle = osThreadNew(StartDmxTask, NULL, &dmxTask_attributes);
